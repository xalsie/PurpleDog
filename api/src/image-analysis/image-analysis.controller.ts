...
export class ImageAnalysisController {
    ...
    analyzeImage(@Body() createImageAnalysisDto: CreateImageAnalysisDto) {
        ...
        console.log(
            '   category type:',
            typeof createImageAnalysisDto.category,
        );
        console.log(
            '   medias type:',
            Array.isArray(createImageAnalysisDto.medias)
                ? 'array'
                : typeof createImageAnalysisDto.medias,
        );
    ...
    async enrichAnalysis(
        ...
        const enrichmentData =
            await this.imageAnalysisService.enrichWithCatawiki(
                analysisId,
                body,
            );
    ...
    getEnrichment(@Param('analysisId') analysisId: string) {
        const enrichment = this.imageAnalysisService.getEnrichment(
            analysisId,
            true,
        );
        if (!enrichment) {
