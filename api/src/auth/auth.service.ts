...
export class AuthService {
    ...
    async registerUser(dto: RegisterUserDto): Promise<void | Error> {
        ...
        try {
            ...
            const proRepo =
                queryRunner.manager.getRepository(ProfessionalProfile);
    ...
    async checkEmail(
        email: string,
    ): Promise<{ exists: boolean; role?: string } | Error> {
        try {
    ...
    async login(loginDto: {
        ...
        const user = await this.userRepository.findOne({
            where: { email },
            relations: ['profile', 'professionalProfile'],
        });
        if (!user) {
    ...
    async reset(token: string, newPassword: string) {
        ...
        const foundUser = await this.userRepository.findOne({
            where: { password: decodedPassword },
        });
        if (!foundUser) {
